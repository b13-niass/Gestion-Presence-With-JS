...
export function apprenantGestion() {
  ...
  const laPromoActive = document.querySelector("#laPromoActive");
  ...
  const findAllApprenants = () => {
    return DB.apprenants.filter((app) => app.promotion == findPromoActive());
  };
  ...
  const buildListApprenants = () => {
    listeApprenantFilter.forEach((apprenant) => {
      tbodyList.insertAdjacentHTML(
        "beforeend",
        templateListApprenant(apprenant)
      );
    });
  };
  ...
  const editApprenantToListeTri = (id, editApprenant) => {
    ...
    listeApprenantForTri.forEach((apprenant, index) => {
      if (apprenant.id == id) {
        listeApprenantForTri[index] = Object.assign(
          {},
          apprenant,
          editApprenant
        );
  ...
  const editApprenantToListeFilter = (id, editApprenant) => {
    ...
    listeApprenantFilter.forEach((apprenant, index) => {
      if (apprenant.id == id) {
        listeApprenantFilter[index] = Object.assign(
          {},
          apprenant,
          editApprenant
        );
  ...
  const eventLoadPopUpEditApprenants = function () {
    ...
    let referentielEdit = formEditApp.querySelector(
  ...
  laPromoActive.innerText = findPromoActive();
  buildSelectRef();
  buildListApprenants();
  eventForAllApprenants();
  eventLoadPopUpEditApprenants();
